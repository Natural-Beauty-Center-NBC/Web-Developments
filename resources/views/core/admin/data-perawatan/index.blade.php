...
<div class="p-4 my-6 mx-4 bg-white rounded-lg shadow md:flex flex-col md:p-6 xl:p-8 dark:bg-gray-800">
    <div class="w-full mb-1">
        <div class="mb-4">
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Daftar Data Perawatan</h1>
        </div>

        <div class="flex flex-row justify-between gap-4 text-right mb-4 mt-8">
            <form action="{{ route('admin.index-perawatan') }}" method="GET">
                <input
                    id="query"
                    type="text"
                    name="query"
            ...
            <div>
                <a href="{{ route('admin.create-perawatan') }}"
                ...
                <tr class="bg-white border-b">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex gap-4">
                            <a href="{{ route('admin.edit-perawatan', ['id' => $item->id]) }}"
                                class="inline-flex gap-2 justify-center items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                Edit
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="w-4 h-4">
                            ...
                            <a href="{{ route('admin.show-perawatan', ['id' => $item->id]) }}"
                                class="inline-flex gap-2 justify-center items-center px-3 py-2 text-sm font-medium text-center text-white bg-green-700 rounded-lg hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                Show
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="w-4 h-4">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <form id="delete-form-{{ $item->id }}" action="{{ route('admin.destroy-perawatan', ['id' => $item->id]) }}" method="POST">
                                @csrf
                                @method('DELETE')
...
<script>
    function confirmDelete(id) {
        Swal.fire({
            ...
            text: "Data Perawatan akan dihapus secara permanen!",
            icon: 'warning',
