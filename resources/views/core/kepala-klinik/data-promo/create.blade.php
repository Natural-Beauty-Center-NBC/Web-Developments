...
@section('content')
<form id="save-form" class="p-8 my-6 mx-4 bg-white rounded-lg shadow" method="POST" action="{{ route('kepala-klinik.store-promo') }}">
    @csrf
    <h3 class="mt-5 flex items-center mb-4 text-lg font-semibold text-gray-900">Tambah Data Promo</h3>
    <div class="w-full flex flex-col gap-4">
        <div class="flex gap-4">
            <div class="w-1/2">
                <label for="kode" class="block mb-2 text-sm font-medium text-gray-900">Kode Promo</label>
                <input name="kode" type="text" id="kode" maxlength="6" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5 @error('kode') border-red-500 @enderror" placeholder="BDAY" required />
                @error('kode')
            ...
            <div class="w-1/2">
                ...
                <input name="jenis" type="text" id="jenis" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5 @error('jenis') border-red-500 @enderror" placeholder="Ulang Tahun" required />
                @error('jenis')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div>
            ...
            <textarea name="keterangan" id="keterangan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5 @error('keterangan') border-red-500 @enderror" placeholder="Bagi customer yang berulangtahun mendapat diskon sebesar 20%" rows="5" required></textarea>
            @error('keterangan')
                <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
            @enderror
        </div>
        <div class="flex gap-4">
            <div class="w-1/2">
                ...
                <input name="nilai_potongan" type="number" id="nilai_potongan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5 @error('nilai_potongan') border-red-500 @enderror" placeholder="Contoh: 20 untuk persen atau 50000 untuk nominal" required />
        ...
        <div class="flex gap-4">
            ...
            <label for="periode" class="block mb-2 text-sm font-medium text-gray-900">Periode</label>
                <input name="periode" type="text" id="periode" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5 @error('periode') border-red-500 @enderror" placeholder="misalnya: 21 April atau - untuk semua tanggal" required />
                @error('periode')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
