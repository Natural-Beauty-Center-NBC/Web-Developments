...
@section('content')
<form id="update-form" class="p-8 my-6 mx-4 bg-white rounded-lg shadow" method="POST" action="{{ route('kepala-klinik.update-promo', ['id' => $promo->id]) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="w-full flex flex-col gap-4">
        <div class="flex gap-4 w-2/3 mb-4">
            <div class="flex-1">
                ...
                <input name="kode" type="text" id="kode" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5" placeholder="BDAY" value="{{ $promo->kode }}" required />
            </div>
            <div class="flex-1">
                <label for="jenis" class="block mb-2 text-sm font-medium text-gray-900">Jenis Promo</label>
                <input name="jenis" type="text" id="jenis" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5" placeholder="Ulang Tahun" value="{{ $promo->jenis }}" required />
            </div>
        </div>
        <div class="w-2/3">
            <label for="keterangan" class="block mb-2 text-sm font-medium text-gray-900">Keterangan Promo</label>
            <input name="keterangan" type="text" id="keterangan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5" placeholder=" Bagi customer yang beruntungan akan mendapatkan diskon 10%" value="{{ $promo->keterangan }}" required />
        </div>
