...
<div class="p-4 my-6 mx-4 bg-white rounded-lg shadow md:flex flex-col md:p-6 xl:p-8 dark:bg-gray-800">
    <div class="w-full mb-1">
        <div class="mb-4">
            <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl">Laporan Top 10 Perawatan Terlaris</h1>
        </div>

        <div class="flex flex-row justify-start gap-8 mb-4 mt-8">
            <div>
                <form method="GET" action="{{ route('kepala-klinik.laporan-perawatan-terlaris') }}" id="inputForm" class="flex gap-3">
                    <select name="year"
            ...
            <div>
                <a href="{{ route('kepala-klinik.download-laporan-perawatan-terlaris',  ['year' => request('year', now()->year), 'month' => request('month', now()->month)]) }}"
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 border" id="dataTable">
            ...
            <tbody>
                ...
                @empty
                <tr>
                    <td colspan="4" class="font-semibold text-center p-6">Tidak ada data untuk ditampilkan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
