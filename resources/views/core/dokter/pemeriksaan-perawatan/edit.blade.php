...
@section('content')
<form id="update-form" class="p-8 my-6 mx-4 bg-white rounded-lg shadow" method="POST" action="{{ route('dokter.update-perawatan', ['id' => $detailPerawatan->id ]) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <h3 class="mt-5 flex items-center mb-4 text-lg font-semibold text-gray-900">Edit Pemeriksaan - {{ $detailPerawatan->transaksi->customer->nama }} [{{ $detailPerawatan->transaksi->no_transaksi }}]</h3>
    <div class="w-full flex flex-col gap-4">
        <div>
            ...
            <select name="perawatan" id="perawatan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5" required>
                ...
                @foreach($perawatans as $item)
                    <option value="{{ $item->id }}" <?= $item->id === $detailPerawatan->perawatan_id ? 'selected' : ''; ?>>{{ $item->nama }}</option>
                @endforeach
            </select>
        </div>
            ...
            <div class="flex-1">
                ...
                <select name="ruangan" id="ruangan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5" required>
                    ...
                    @foreach($ruangans as $item)
                        <option value="{{ $item->id }}" <?= $item->id === $detailPerawatan->transaksi->ruangan_id ? 'selected' : ''; ?>>
                            {{ $item->no_ruangan }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex-1">
                ...
                <select name="beautician" id="beautician" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#FF9EAA] focus:border-[#FF9EAA] block w-full p-2.5" required>
                    ...
                    @foreach($beauticians as $item)
                        <option value="{{ $item->id }}" <?= $item->id == $detailPerawatan->transaksi->beautician_id ? 'selected' : ''; ?>>
                            {{ $item->nama }}
                        </option>
                    @endforeach
                </select>
        ...
        <input type="hidden" id="id_transaksi" name="id_transaksi" value="{{ $detailPerawatan->transaksi->id }}" />
...
<script>
    function confirmUpdate() {
        Swal.fire({
            ...
            text: "Data Detail Perawatan akan diubah!",
            icon: 'warning',
